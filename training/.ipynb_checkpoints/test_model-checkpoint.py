...
import time
from training.mae_inference import VideoClassifier



def main(test_csv_path, dataset_path, output_file, model_details, saved_model_dir=None):
    """
    ...
    if os.path.exists(os.path.join(saved_model_dir,"best_model")): 
        new_saved_model_dir = os.path.join(saved_model_dir, "best_model")
        print("================ using the best model in testing ==================")
        video_claasificer = VideoClassifier(model_path = new_saved_model_dir)
    else:      
        new_saved_model_dir = os.path.join(saved_model_dir, "latest_checkpoint")
        print("================ using the latest model in testing ==================")
        video_claasificer = VideoClassifier(model_path = new_saved_model_dir)  
        
    new_saved_model_dir = "/home/jupyter/rack/mae_video_classification_pod/models_long_hanging/videomae-base-2501445763862/best_model"
    video_claasificer = VideoClassifier(model_path = new_saved_model_dir) 
    ...
    for resource_id, gcs_filename, actual_label in zip(resource_ids, gcs_filenames, actual_labels):
        ...
        file_path = os.path.join(dataset_path, actual_label, os.path.basename(gcs_filename))

        predicted_label = video_claasificer.get_top_label(video_path = file_path) 
        score = 100
